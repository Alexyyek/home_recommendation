...
def read_input(topN, seperator='\t'):
    ...
    for line in sys.stdin:
        ...
        if current_resblock == resblock_alpha:
            ...
            if current_resblock:
                get_resblock_top(topN, current_resblock, resblock_dict)
            current_resblock = resblock_alpha
            resblock_dict.clear()
            resblock_dict[resblock_beta] = similarity
    if current_resblock == resblock_alpha:
        get_resblock_top(topN, resblock_alpha, resblock_dict)
...
def get_resblock_top(topN, current_resblock, resblock_dict):
    ...
    resblock_dict = resblock_dict[0:topN]
    for resblock in resblock_dict:
        outpt_str = '{current_resblock}\t{resblock}'.format(
